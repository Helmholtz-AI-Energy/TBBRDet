...
Config files:
```
├── mask_rcnn_swin-t-p4-w7_fpn_ms-crop-3x_coco.py                          <- Base model setup, everything eventually inherits this. Trains from scratch
├── mask_rcnn_swin-t-p4-w7_fpn_fp16_ms-crop-3x_coco.py                     <- FP16 version of base model
├── mask_rcnn_swin-t-p4-w7_fpn_fp16_ms-crop-3x_coco_ablation.py            <- Modifies FP16 base model to handle RGBT inputs
├── mask_rcnn_swin-t-p4-w7_fpn_fp16_ms-crop-3x_coco.scratch.py             <- Train from scratch, actually just inherits FP16 base model
├── mask_rcnn_swin-t-p4-w7_fpn_fp16_ms-crop-3x_coco.scratch_ablation.py    <- Train from scratch with ablation, actually just inherits the base ablation model
├── mask_rcnn_swin-t-p4-w7_fpn_fp16_ms-crop-3x_coco.pretrained.py          <- Train from scratch, actually just inherits FP16 base model
├── mask_rcnn_swin-t-p4-w7_fpn_fp16_ms-crop-3x_coco.pretrained_ablation.py <- Modifies FP16 base model to handle RGBT inputs
└── mask_rcnn_swin-t-p4-w7_fpn_fp16_ms-crop-3x_coco.tmp.py                 <- Deprecated
```
Note that the naming is `model_definition{.experiment}.py`, where the ones with `{experiment}` are those that are actually used for training runs in the end, the others are just base models to inherit.

# From `mmdetection` repo
