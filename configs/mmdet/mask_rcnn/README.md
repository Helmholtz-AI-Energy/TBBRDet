...
## Structure
```
├── mask_rcnn_r50_caffe_fpn_mstrain-poly_3x_thermals.py            <- Deprecated
├── mask_rcnn_r50_fpn_mstrain-poly_3x_coco.py                      <- Base model setup, everything eventually inherits this
├── mask_rcnn_r50_fpn_mstrain-poly_3x_coco_ablation.py             <- Modifies base model to handle RGBT inputs
├── mask_rcnn_r50_fpn_mstrain-poly_3x_coco.pretrained.py           <- 3x schedule fine-tuning for pretrained model
├── mask_rcnn_r50_fpn_mstrain-poly_3x_coco.scratch.py              <- 3x training from scratch, actually just inherits base model and that's it
├── mask_rcnn_r50_fpn_mstrain-poly_3x_coco.pretrained_ablation.py  <- Ablation version of pretrained fine-tuning
└── mask_rcnn_r50_fpn_mstrain-poly_3x_coco.scratch_ablation.py     <- Ablation version of scratch training
```
